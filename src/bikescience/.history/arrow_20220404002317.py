...
def create_arc(N, R, xc, yc, start_point, end_point, reverse=False, self_arc = False):
    """
    ** Internal use function **
    ...
    print(start_t, end_t)
    if reverse:
        if not self_arc: start_t = (start_t + 2*math.pi) % (2*math.pi)
...
def draw_arrow(folium_map, lat1, lon1, lat2, lon2, color='blue', weight=3, tip=6, text='', radius_fac=1.0, reverse=False):
    ...
    """
    global N
    lat1, lon1, lat2, lon2 = .99*lat1 + .01*lat2, .99*lon1 + .01*lon2, .99*lat2 + .01*lat1, .99*lon2 + .01*lon1
    ...
    arc_x, arc_y, start, end = create_arc(N, RADIUS, xc, yc, [lon1, lat1], [lon2, lat2], reverse=False, self_arc = reverse)
    arc2_x, arc2_y, start2, end2 = create_arc(N, RADIUS, xc, yc, [lon1, lat1], [lon2, lat2], reverse=True, self_arc = reverse)
    print(start, end)
    print(start2, end2)
    if ((end2 - start2) < (end - start)) ^ reverse:
