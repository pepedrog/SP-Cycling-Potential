...
function when_clicked(trip) {
    return function (e) {
        if (topografia.style.display == 'none' || current_trip_id != trip.id) {
...
function validate_evaluation() {
    const distance = validate_radio('dist')
    const incl = validate_radio('incl')
    const final_eval = valudate_radio('final_eval')

    if (!!distance || !!incl || !!final_eval) {
        document.getElementsById('valida_avaliacao').style.display = "block"
        return false
    }

    document.getElementsById('valida_avaliacao').style.display = "none"
    return true
}
