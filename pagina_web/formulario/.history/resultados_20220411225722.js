...
function get_answers(frequency = 'Geral', re_plot = false) {
    for (const key of regions) {
        for (const route of routes_per_region[key]) {
...
function iter_answers(id, frequency) {
    ...
    for (const answer of answers) {
        for (const a of answer.avaliacoes) {
            if (a.id == id && (frequency == 'Geral' && a.frequencia == frequency)) {
                count++
...
function filter_answers(frequency) {
    let dist_potentials_aux = dist_potentials
    let incl_potentials_aux = incl_potentials
