...
function get_potentials() {
    for (const key of regions) {
        for (const route of routes_per_region[key]) {
            ids_regions.push(route.id)
            dist_pontentials.push(route.dist_pot)
            incl_pontentials.push(route.incl_pot)
...
function plot_scatter(x, y, title, element) {
    ...
    let line = linear_approx(join_array(x), join_array(y), x.length * x[0].length)
...
*/
    ...
    let answers_potentials = get_potentials()
}
