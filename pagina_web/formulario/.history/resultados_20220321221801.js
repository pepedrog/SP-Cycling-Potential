...
function get_potentials() {
    for (const key of regions) {
        for (const route of routes_per_region[key]) {
            ...
            dist_pontentials.push(Number(route.dist_pot))
            incl_pontentials.push(Number(route.incl_pot))
    ...
    for (const answer of answers) {
        for (const a of answer.avaliacoes) {
            if (a.id == id) {
                ...
                answers_dist.push(a.dist)
                answers_incl.push(a.incl)
                index = ids_regions.indexOf(a.id)  
                answers_dist_pot.push(dist_pontentials[index])
                answers_incl_pot.push(incl_pontentials[index])
            }
        }
    }
...
*/
    plot_scatter(dist_pontentials, dist_mean_ans, 'distância', 'dist_mean_answers_chart')
    plot_scatter(incl_pontentials, incl_mean_ans, 'inclinação', 'incl_mean_answers_chart')
