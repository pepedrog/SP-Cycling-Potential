...
function configure_form() {
    ...
    const checkboxs_regions = document.getElementsByName('motivo')
    for (let reason of checkboxs_regions) {
        reason.addEventListener('change', (event) => {
            if (event.currentTarget.checked) {
                selected_reasons.push(region.value);
            } else {
                const index = selected_reasons.indexOf(region.value);
                if (index > -1) {
...
function change_page(direction = 1) {
    ...
    if (page == 1 && !(form.checkValidity() && validate_reason() && validade_regions())) {
...
function when_clicked(trip) {
    return function (e) {
        if (topografia.style.display == 'none') {
