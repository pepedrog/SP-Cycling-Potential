...
def create_arc(N, R, xc, yc, start_point, end_point, reverse=False, loop_arc = False):
    """
    ** Internal use function **
    ...
    if reverse:
        if not loop_arc: start_t = (start_t + 2*math.pi) % (2*math.pi)
...
def draw_arrow(folium_map, lat1, lon1, lat2, lon2, color='blue', weight=3, tip=6, text='', radius_fac=1.0, loop_arc = False):
    """
    ...
    arc_x, arc_y, start, end = create_arc(N, RADIUS, xc, yc, [lon1, lat1], [lon2, lat2], reverse=False, loop_arc=loop_arc)
    arc2_x, arc2_y, start2, end2 = create_arc(N, RADIUS, xc, yc, [lon1, lat1], [lon2, lat2], reverse=True, loop_arc=loop_arc)

    if ((end2 - start2) < (end - start)) ^ loop_arc:
