...
def create_arc(N, R, xc, yc, start_point, end_point, reverse=False):
    ...
    ** Internal use function **
    ...
    if reverse:
        start_t = (start_t + math.pi) % (2*math.pi)
        end_t = (end_t + math.pi) % (2*math.pi)
...
def draw_arrow(folium_map, lat1, lon1, lat2, lon2, color='blue', weight=3, tip=6, text='', radius_fac=1.0, counterclockwise = False):
    """
    ...
    if (((end2 - start2) > (end - start)) ^ counterclockwise):
