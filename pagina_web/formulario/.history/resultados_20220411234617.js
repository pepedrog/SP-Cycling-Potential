...
function plot_comparison_scatter() {
    ...
    for (f of ['Diariamente', 'Frequentemente', 'Eventualmente', 'Raramente', 'Nunca'])
    {
        ...
        let line = linear_approx(dist_potentials, dist_mean_ans, answers_count, answers_count.length)
        let point = get_endpoints(line[0], line[1])
        let trace_d = {
            x: [point[0][0], point[1][0]],
            y: [point[0][1], point[1][1]],
            mode: 'lines',
            type: 'scatter',
            name: f + ' - LSM: ' + (Math.round(line[0] * 100) / 100)
        };

        line = linear_approx(incl_potentials, incl_mean_ans, answers_count, answers_count.length)
        point = get_endpoints(line[0], line[1])
        let trace_i = {
            x: [point[0][0], point[1][0]],
            y: [point[0][1], point[1][1]],
            mode: 'lines',
    ...
    Plotly.newPlot('incl_answers_chart', traces_dist, layout);
}
